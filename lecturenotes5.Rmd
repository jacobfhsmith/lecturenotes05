...
- Homework #01 due Thursday 2-04 at 11:59 PM
- Homework #02 assigned Thursday 2-04
- Lab #03 tomorrow
...
the attached html file for gifs with animations of these joins.
...
Join flights to airports. Note these two datasets have no variables in common
...
(2) Create a new dataset by joining `dest_delays` and `airports`. Only include observations that have both delay and airport information. Note `dest_delays`
...
dataset you created by joining `dest_delays` and `airports`? Use an appropriate join function to investigate this issue and determine what is going on here.

There are 105 airports in the `dest_delays` dataset, but when we join to 
airports there are only 101 airports. There are four airports in the 
